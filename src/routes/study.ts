...
router.post('/', verifyToken, async function (req: Request, res: Response) {
  const { topic, qnt_reviews, date, user_id } = req.body;

  try {
    if (!topic || !qnt_reviews || !date) {
    ...
    for (let i = 0; i < qnt_reviews; i++) {
      let reviewDate;
      if (i === 0) {
        reviewDate = addDays(parsedDate, 1);
      } else if (i === 1) {
        reviewDate = addWeeks(parsedDate, 1);
      } else {
        reviewDate = addMonths(parsedDate, i - 1);
      }
